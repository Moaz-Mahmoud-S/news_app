  ...
  Widget build(BuildContext context) {
    return BlocConsumer <NewsAppCubit , NewsAppStates>(
        ...
        builder: (BuildContext context,NewsAppStates state){return Conditional.single(
            context: context,
            conditionBuilder: (BuildContext context) => NewsAppCubit.sports.length>0,
            widgetBuilder: (BuildContext context){ return
              ListView.separated(
                  physics: BouncingScrollPhysics(),
                  itemBuilder: (BuildContext context,int index){return buildnewsitem(NewsAppCubit.sports[index],context);}, separatorBuilder: (BuildContext context,int index){return Padding(
                padding: const EdgeInsets.only(right: 10,left: 10),
                child: Container(height: 1,color: Colors.grey,),
              );}, itemCount: NewsAppCubit.sports.length);},
